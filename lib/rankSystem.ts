...
export function calculateLevelRequirements(currentLevel: number): {
  ...
  // העלנו את כמות הנקודות הנדרשות - כעת זה 600 נקודות בסיס במקום 100
  // זה הופך את עליית הרמה למאתגרת יותר ומדרבן לשחק יותר
  return {
    pointsNeeded: Math.floor(600 * baseMultiplier), // הוגדל מ-100 ל-600 (פי 6)
    gamesNeeded: Math.floor(5 * baseMultiplier),
    winsNeeded: Math.floor(3 * baseMultiplier),
    achievementsNeeded: Math.max(1, Math.floor(2 * baseMultiplier)) // לפחות 1, לא 0
  };
}
